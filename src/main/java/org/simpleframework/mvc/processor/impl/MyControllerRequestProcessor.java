 ...
 **/
@Slf4j
public class MyControllerRequestProcessor implements MyRequestProcessor {
    //IOC容器
     ...
     */
    public MyControllerRequestProcessor() {
    ...
    @Override
    public boolean process(MyRequestProcessorChain requestProcessorChain) throws Exception {
        ...
        if (controllerMethod == null) {
            requestProcessorChain.setResultRender(new MyResourceNotFoundResultRender(method, path));
            return false;
        }
     ...
     * 根据不同情况设置不同的渲染器
     */
    private void setResultRender(Object result, ControllerMethod controllerMethod, MyRequestProcessorChain requestProcessorChain) {
        if (result == null) {
            return;
        }
        MyResultRender resultRender;
        boolean isJson = controllerMethod.getInvokeMethod().isAnnotationPresent(ResponseBody.class);
        if (isJson) {
            resultRender = new MyJsonResultRender(result);
        } else {
            resultRender = new MyViewResultRender(result);
        }
        requestProcessorChain.setResultRender(resultRender);
    }

    private Object invokeControllerMethod(ControllerMethod controllerMethod, MyRequest request) {
